...
$(document).ready(function () {
  ...
  var scrollLastSectionPosition = 0; // Callbacks that are triggered when setCurrentTopic() is called.
  ...
  function handleSkipClick(event) {
    ...
    var sectionId = this.getAttribute('data-section-id'); // get the topic & section indexes
    ...
    if (section.exercises.length && !section.completed && !section.allowSkip) {
      ...
      sectionSkipped([section.jqElement]); // notify server
  ...
  function buildTopicsList() {
    ...
    topicsDOM.each(function (topicIndex, topicElement) {
      ...
      var jqTopic = $('<div class="topic" index="' + topicIndex + '">' + topic.titleText + '</div>');
  ...
  Shiny.inputBindings.register(topicMenuInputBinding, 'learnr.topicMenuInputBinding'); // continueButtonInputBinding
  // ------------------------------------------------------------------
  // This keeps tracks of what topic is selected
  ...
  Shiny.inputBindings.register(continueButtonInputBinding, 'learnr.continueButtonInputBinding'); // transform the DOM here
  ...
  function transformDOM() {
    ...
    var tutorialTitle = $("<h2 class=\"tutorialTitle\">".concat(titleText, "</h2>"));
    tutorialTitle.on('click', showFloatingTopics);
    $('.topics').prepend(tutorialTitle);
    $('.bandContent.topicsListContainer').append(buildTopicsList()); // initialize visibility of all topics' elements
  ...
  function handleLocationHash() {
    ...
    setCurrentTopic(findTopicIndexFromHash()); // navigate to a topic when the history changes
  ...
  function sectionCompleted(section) {
    ...
    if (topicCompleted) {
      ...
      if (!_section.completed) {
        ...
        updateTopicProgressBar(topicIndex); // update the exercise

        $(_section.jqElement).addClass('done');
        _section.completed = true; // update visibility of topic's exercises and actions
  ...
  function sectionSkipped(exerciseElement) {
    ...
    updateTopicProgressBar(topicIndex); // update visibility of topic's exercises and actions

    updateVisibilityOfTopicElements(topicIndex);
  }

  transformDOM();
  handleLocationHash(); // initialize components within tutorial.onInit event
