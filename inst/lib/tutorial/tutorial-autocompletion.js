...
function TutorialCompleter(tutorial) {
  ...
  this.$onChange = function (data) {
    ...
    data = data || {}; // only perform live autocompletion while user
    ...
    var line = this.session.getLine(pos.row); // NOTE: we allow new autocompletion sessions following a
  ...
  function initializeAceEventListeners(editor) {
    ...
    }.bind(editor); // register our handlers on the Ace editor
    ...
    editor.completers.push({
      getCompletions: function getCompletions(editor, session, position, prefix, callback) {
        ...
        self.$tutorial.$serverRequest('completion', payload, function (data) {
          data = data || []; // define a custom completer -- used for e.g. automatic
          ...
          var completer = {
            insertMatch: function insertMatch(editor, data) {
              ...
              editor.execCommand('insertstring', term); // move cursor backwards for functions
  ...
  function ensureInitialized(editor) {
    ...
    initializeSetupChunk(editor); // generate a live autocompleter for this editor if
    // not yet available
  ...
  function autocomplete(event) {
    ...
    ensureInitialized(editor); // cancel any pending live autocompletion
