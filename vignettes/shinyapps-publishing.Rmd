...
Suppose you are teaching a statistical programming seminar over 5 weeks for several class sections of 60 people each. The course is short and students have just been introduced to R and RStudio, so it may not make to set up a full server for tutorials, or ask students to configure their own R package. 
...
In my use case, I had my department purchase the Standard shinyapps.io subscription for $100/month, which gave me **2,000 Active Hours** per month. (Note that **Active Hours** refers to the amount of time spent launching the tutorial and actively processing R code and input submitted by students, not the total amount of time the browser is open on a student's computer.)
...
We used around 1000 Active Hours in our first month, and around 300 in our second month of the course, so next year, I will likely purchase the Standard subscription for one month, and reduce to the Basic level in the second month.
...
For consecutive classes, we still had some issues with open server connections from the previous class getting in the way. I published an identical learnr tutorial with a different name (i.e. `"tutorial-2"`), so I could direct some students to a different link as needed.
