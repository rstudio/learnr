...
initialize_session_state <- function(session, metadata, location, request) {
  ...
  initialize_identifer <- function(identifier, default) {
    ...
    header <- as_rook_header(getOption(sprintf("teachdown.http_header_%s", identifier)))
    ...
    write_request(session, sprintf("teachdown.%s", identifier), value)
  ...
  write_request(session, "teachdown.http_location", location)
