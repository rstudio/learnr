...
<script type="text/javascript">loadSnippet('tutorialyaml-progressive')</script>

You can also specify this option on a per sub-topic bases using the `data-progressive` attribute. For example, the following code enables progressive rendering for a single sub-topic:




You can also use this attribute to disable progressive rendering for a topic if the global `progressive` option is `TRUE`.
