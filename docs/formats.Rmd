...
You may want to allow users of your tutorials to skip through exercises that they can't quite figure out the right to. This might especially be true if you are using `progressive` option and you want users to be able to optionally see the next exercise even if they haven't completed the prior one.

If you specify the `allow_skip` option then all exercises will include a **Skip** button that will mark the exercise as skipped and allow progression to the next one. For example:
