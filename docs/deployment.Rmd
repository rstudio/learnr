...
```r
install.packages(c("tutor", "checkthat"))
tutor::run_tutorial("introduction", package = "mypackage")
```
...
As part of deploying a tutorial you might want to record the various user events which occur witihn tutorials (e.g. exercise and question submissions, requests for hints/solutions, etc.). 

### Event Recorder Function

You can capture events by using a custom eent recorder function. This function is specified via the `tutor.event_recorder` global option. For example, here's how you would define a simple event recorder function that prints to stdout:

<div id="tutorrecorder"></div>
<script type="text/javascript">loadSnippet('tutorrecorder')</script>

The `tutorial_id` parameter provides a unique identifier for the tutorial (defaults to the directory containing the tutorial Rmd).

The `tutorial_version` parameter provides the version number of the tutorial.

The `user_id` parameter provides a unique identifer for the tutorial user (defaults to the username of the currently logged in user).
...
The `data` parameter is an R list which provides additonal data that varies depending on which `event` is being recorded.

### Tutorial and User IDs

By default the tutorial ID is the directory containing the tutorial Rmd and user ID is the username of the local account executing the tutorial. While these defaults work well for local/package deployment, if you are hosting tutorials on a server you may want to customize these values based on other considerations.

You can pass a custom tutorial ID and/or user ID through to the **tutor** package by adding HTTP headers to the requests that route to the tutorial. The names of the headers are configurable, and should be specified using the `tutor.http_header_tutorial_id` and `tutor.http_header_user_id` global options. For example:
