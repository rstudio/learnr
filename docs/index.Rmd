...
Here are some simple examples of tutorials created with the **rtutor** package:
...
To create a tutorial, just use `library(rtutor)` within your Rmd file to activate tutorial mode, then use the `exercise = TRUE` attribute to turn code chunks into exercises. Users can edit and execute the R code and see the results right within their browser.
...
1. Install the development version of the **rtutor** package from GitHub as follows:
...
While you can use tutorial elements many different R Markdown formats (see the article on [Tutorial Formats](formats.html) for details), the **rtutor** package also includes a custom `rtutor::tutorial` format which provides some default layout and behavior (including progress tracking) which you'll likely want to use in many of your tutorials. 
...
If your tutorial is included within an R package you can also run it via the `rtutor::run_tutorial` function. For example, you can run a live version of the "Hello, Tutor" example provided above with:
...
The **rtutor** package uses `runtime: shiny_prerendered` to turn regular R Markdown documents into live tutorials. Since tutorials are Shiny applications at their core, it's also possible to add other forms of interactivity using Shiny (e.g. for teaching a statistical concept interactively). 
...
The most straightforward way to deploy a tutorial is to include it within an [R package](publishing.html#r-package) and have users run it directly via the `rtutor::run_tutorial` function.
