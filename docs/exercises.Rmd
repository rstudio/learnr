...
Note that this feature is only available if you are using the [`rtutor::tutorial`](formats.html#tutorial-format) R Markdown format (other custom formats may have their own internal mechanisms for progressive reveal).
...
The **rtutor** package doesn't directly include features for checking exercise inputs however it does include lower-level hooks that enable other packages to provide tools for exercise checking. You can provide an external function for exercise checking by setting the `exercise.checker` knitr chunk option within the `setup` chunk of your tutorial, then adding a "-check" chunk for any exercise you want to check. For example:
...
The **[checkr](https://github.com/dtkaplan/checkr)** package currently provides code checking functions that are compatible with **rtutor**. 
